// app/index.tsx — agrega la función Paso7
import {
  useEffect,
  useRef, useState,
} from 'react'
...
import { useSincronizacionPrecios } from '../hooks/useSincronizacionPrecios'
...
export function Paso7() {
  ...
  const { estado, intentos, latencia, reconectar, reiniciar } =
    useSincronizacionPrecios('servicio-precios')
  ...
  // ── Sincronización periódica de eventos (useEffect + setInterval) ─
  ...
  useEffect(() => {
    ...
    const intervalo = setInterval(() => {
      ...
      const eventos = [
        `[${ahora}] Precio de Café Nacional 500g actualizado en Supermaxi`,
        `[${ahora}] Nuevo producto agregado al catálogo: Yogurt Griego 1L`,
        `[${ahora}] Alerta de precio enviada a 12 usuarios`,
        `[${ahora}] Comercio Tía sincronizó 340 precios`,
      ]
  ...
  return (
    <View style={[styles.contenedor, { backgroundColor: colores.fondo }]}>
      ...
      <Text style={[styles.titulo, { color: colores.texto }]}>
        Estado del servicio de precios
      </Text>
      ...
      <View style={[
        ...
        <Text style={[styles.detalle, { color: colores.detalle }]}>
          PreciosEC API · GET /kache/productos/
        </Text>
      ...
      {/* Filtro de eventos con foco automático (useRef) */}
      <TextInput
        ref={inputFiltroRef}
        style={[
          styles.input,
        ...
        {estado !== 'conectado' ? (
          <Text style={styles.logVacio}>
            {estado === 'desconectado' || estado === 'error'
              ? 'Sin conexión — los eventos aparecen al conectar'
